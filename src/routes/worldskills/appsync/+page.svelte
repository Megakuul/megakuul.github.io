...
<svelte:head>
  <title>AppSync | Megakuul</title>
  <meta
    name="description"
    content="AppSync JavaScript resolvers: authorization, pipelines, DynamoDB, Lambda, HTTP, RDS, EventBridge, OpenSearch and SQS."
  />
  <meta property="og:title" content="AppSync - Megakuul" />
  <meta
    property="og:description"
    content="Copyable AppSync JavaScript for unit resolvers, pipelines and data source integrations."
  />
  <link rel="canonical" href="https://megakuul.ch/worldskills/appsync" />
</svelte:head>
...
<div class="sticky top-0 z-30 border-b border-white/5 bg-[rgb(17,16,16)]/90 backdrop-blur-md">
  <div class="mx-auto flex max-w-350 items-center gap-3 px-4 py-3">
    ...
    >
    <h1 class="min-w-0 flex-1 truncate text-lg font-bold">AppSync</h1>
    <button
      ...
      <section aria-labelledby={group.id} class="space-y-4">
        ...
        {#each group.snippets as snippet}
          <article id={snippet.id} class="scroll-mt-24 space-y-3">
            ...
            {#each snippet.blocks as block, i}
              {@render code(`${snippet.id}-code-${i}`, block.title, block.code, block.html)}
            {/each}
            {#if snippet.extra.length}
              <details class="rounded-xl border border-white/10">
                <summary class="cursor-pointer px-4 py-3 text-sm font-semibold">Example</summary>
                <div class="space-y-3 px-3 pb-3">
                  {#each snippet.extra as block, i}
                    {@render code(`${snippet.id}-extra-${i}`, block.title, block.code, block.html)}
                  {/each}
                </div>
              </details>
            {/if}
