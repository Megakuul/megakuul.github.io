...
export const handler = async () => {
  ...
  const db = await mysql.createConnection({
    ...
    authPlugins: { mysql_clear_password: () => () => Buffer.from(token + '\0') },
    connectTimeout: 5000,
  });
  try {
    await db.query(
      'CREATE TEMPORARY TABLE demo_items (id integer PRIMARY KEY, name varchar(100), price decimal(10,2)) ENGINE=InnoDB',
    );
    await db.beginTransaction();
    await db.execute('INSERT INTO demo_items VALUES (?,?,?),(?,?,?)', [
      1,
      'Apple',
      2.5,
      2,
    ...
    await db.execute('INSERT INTO demo_items VALUES (?,?,?) ON DUPLICATE KEY UPDATE price = ?', [
      1,
      'Apple',
      4,
      4,
    ...
    const [rows] = await db.execute(
      'SELECT * FROM demo_items WHERE price >= ? ORDER BY id LIMIT 10 OFFSET 0',
      [2],
    );
